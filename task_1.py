...
class Edrone():
	...
	def altitude_set_pid(self):
		self.Kp[2] = 1000 * 0.06 # This is just for an example. You can change the ratio/fraction value accordingly
		self.Ki[2] = 0 * 0.008
		self.Kd[2] = 600 * 0.3
	...
	#	8. Add error_sum
		...
		def throttle_pid():
			...
			self.cmd.rcThrottle=int(1500+self.throttle_error*1000*0.06+(self.throttle_error-self.throttle_prev_error)*600*0.3+ self.throttle_sum_error*0)
		...
		def pitch_pid():
			...
			self.cmd.rcPitch=int(1500-(self.pitch_error*40*0.06+(self.pitch_error-self.pitch_prev_error)*50*0.3+ self.pitch_sum_error*0)/self.sample_time)
		...
		def roll_pid():
			...
			self.cmd.rcRoll=int(1500+(self.roll_error*40*0.06+(self.roll_error-self.roll_prev_error)*50*0.3+ self.roll_sum_error*0)/self.sample_time)
